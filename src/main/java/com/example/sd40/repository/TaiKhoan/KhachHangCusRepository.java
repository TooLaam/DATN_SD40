...
public interface KhachHangCusRepository extends JpaRepository<KhachHang,Long> {
    ...
    @Query("select ghct from GioHang gh join GioHangDetail ghct on gh.id = ghct.gioHang.id join ChiTietSanPham ctsp on ctsp.Id = ghct.chiTietSanPham.Id where gh.khachHang.id = ?1")
