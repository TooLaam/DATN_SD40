...
public interface KhachHangCusRepository extends JpaRepository<KhachHang,Long> {
    ...
    @Query("select ghct from Gio_hang gh join GioHangChiTiet ghct on gh.id = ghct.gioHang.id join ChiTietSanPham ctsp on ctsp.Id = ghct.chiTietSanPham.Id where gh.khachHang.id = ?1")
