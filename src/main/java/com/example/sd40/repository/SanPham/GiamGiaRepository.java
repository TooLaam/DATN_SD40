...
public interface GiamGiaRepository extends JpaRepository<GiamGIa, Long> {
    ...
    @Query("select sp.id,ct.giaHienHanh,(ct.giaHienHanh*(100-gg.mucGiam))/100 from SanPham sp join GiamGIa gg on sp.giamGIa.Id= gg.Id join ChiTietSanPhamMauSacHinhAnh ct on sp.id=ct.sanPham.id where sp.id=?1 and ct.mau_sac.Id=?2")
