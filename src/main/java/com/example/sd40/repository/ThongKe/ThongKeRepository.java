...
public interface ThongKeRepository extends JpaRepository<HoaDon,Long> {
    @Query(value = "SELECT" +
            ...
            " FROM hoa_don where ngay_cap_nhat >= :ngayBatDau and ngay_cap_nhat<= :ngayKetThuc", nativeQuery = true)
    Object TongDonHangAll(@Param("ngayBatDau") String ngayBatDau, @Param("ngayKetThuc") String ngayKetThuc);


    @Query(value = "select sum(hoa_don_chi_tiet.so_luong) from hoa_don join hoa_don_chi_tiet on hoa_don.id = hoa_don_chi_tiet.hoa_don_id  where hoa_don.trang_thai not in (5) and ngay_cap_nhat >= :ngayBatDau and ngay_cap_nhat <= :ngayKetThuc", nativeQuery = true)
    Integer SanPhamDaBanAll(@Param("ngayBatDau") String ngayBatDau, @Param("ngayKetThuc") String ngayKetThuc);

    @Query(value = "select sum(hoa_don.tong_tien) from hoa_don where ngay_thanh_toan IS NOT NULL and ngay_thanh_toan >= :ngayBatDau and ngay_thanh_toan <= :ngayKetThuc", nativeQuery = true)
    Object DoanhThu(@Param("ngayBatDau") String ngayBatDau, @Param("ngayKetThuc") String ngayKetThuc);
    ...
    @Query(value = "SELECT" +
            ...
            " WHERE ngay_thanh_toan IS NOT NULL and YEAR(ngay_thanh_toan) = ?1" +
